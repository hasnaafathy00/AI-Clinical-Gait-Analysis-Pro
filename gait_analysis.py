...
def main():
    ...
    csv_path = os.path.join(args.out, f"{safe_name}_gait_data.csv")
    df.to_csv(csv_path, index=False)

    # Save report
    report_path = os.path.join(args.out, f"{safe_name}_Final_Master_Report.txt")
    ...
    plt.tight_layout()
    graph_path = os.path.join(args.out, f"{safe_name}_Full_Grafts.png")
    fig.savefig(graph_path)

    # --- 6. Gait-cycle normalization (ensemble-averaged curves) ------------
    ...
    norm_path = os.path.join(args.out, f"{safe_name}_Normalized_Cycles.png")
    norm_fig.savefig(norm_path)

    # --- 7. Combined PDF report -------------------------------------------
    pdf_path = os.path.join(args.out, f"{safe_name}_Report.pdf")
